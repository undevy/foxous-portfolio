    ...
    <div className="bg-white rounded-3xl shadow-sm border border-gray-200 p-4">
      <div className="max-w-screen-xl mx-auto flex justify-between items-center">
        ...
        <div className="flex space-x-3">
          {/* Центральные иконки всех компаний */}
          {Object.keys(companyData).map(companyId => (
            <button 
              key={companyId}
              onClick={() => toggleCompany(companyId)}
              className={`w-10 h-10 ${activeCompany === companyId ? 'bg-blue-100 text-blue-600' : 'bg-gray-100 text-gray-500'} rounded-lg flex items-center justify-center hover:bg-blue-200`}
              title={companyData[companyId].name}
            >
              {companyIcons[companyId]}
            </button>
          ))}
        </div>
